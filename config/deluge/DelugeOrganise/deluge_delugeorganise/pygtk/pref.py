...
class DelugeOrganiseConfig(object):
    """
    ...
    def load(self):
        log.debug('Adding Deluge Organise Preferences page')
        builder = Builder()
        builder.add_from_file(get_resource('pref.ui'))

        self.plugin.add_preferences_page(
            ('DelugeOrganise'), builder.get_object('label_prefs_box')
        )
    ...
    def unload(self):
        self.plugin.remove_preferences_page(('DelugeOrganise'))
