...
class Torrent:
  ...
  def diff_run(self, interval_diff):
    ...
    if self.seeder.torrent.total_size > self.small_torrent_limit:
      if self.total_downloaded > self.threshold: 
        ...
        if randomint % 3 == 0:
          uploaded_bytes = int(self.seed_per_second * self.announce_interval * random.uniform(0.25, 0.65))
        elif randomint % 4 == 0:
      ...
      randomint = random.randint(0, 10) 
      download_bytes = int(self.seed_per_second * self.announce_interval * random.uniform(0.71, 1.61))

      if randomint % 3 == 0:
        download_bytes = int(self.seed_per_second * self.announce_interval * random.uniform(0.55, 0.77))
      elif randomint % 4 == 0:
        download_bytes = int(self.seed_per_second * self.announce_interval * random.uniform(0.99, 1.12))
      elif randomint % 7 == 0:
        download_bytes = 0        

      self.total_downloaded += download_bytes

      if self.total_downloaded > self.seeder.torrent.total_size:
        self.total_downloaded = self.seeder.torrent.total_size
        self.session_downloaded += 0
      else:
        self.session_downloaded += download_bytes
    else:
      self.session_uploaded += 0
      self.total_uploaded += 0
      self.session_downloaded += 0
  ...
  def announce(self):
    try:
      self.seeder.upload(self.session_uploaded, self.session_downloaded, self.seeder.torrent.total_size - self.total_downloaded)      
...
class Manager:
  ...
  def remove_completed(self):
    completed = iptcheck.get_done()

    try:
      for instance in list(self.torrents):
        for complete in completed:
          lower = instance.seeder.torrent.name.lower()
          if lower.find(complete):
            print("Unlinked " + lower)
            if instance in self.torrents:
              self.torrents.remove(instance)
            if os.path.exists(instance.filepath):
              os.unlink(instance.filepath)
              os.unlink(instance.filepath + ".progress")
    except:
      traceback.print_exc(file=sys.stdout) 
